...
import re
import traceback
def test_answer_for_reward(pred_str, ans_str,test_data_type="mnli"):
...
class DeepSpeedPPOTrainer():
    ...
    def _generate_sequence(self, prompts, mask, step, temperature=0.8,do_sample=False):
        ...
        if self.args.print_answers and (step % self.args.print_answers_interval == 0):
            ...
            with open(filename, 'a', encoding='utf-8') as file:
                p = f"--- prompt --> step={step}, rank={torch.distributed.get_rank()}, {self.tokenizer.batch_decode(prompts, skip_special_tokens=True)}\n"
                a = f"--- ans    --> step={step}, rank={torch.distributed.get_rank()}, {self.tokenizer.batch_decode(ans, skip_special_tokens=True)}\n"
    ...
    def generate_experience(self, prompts, mask, step, 
        ...
        seq, ans_len = self._generate_sequence(prompts, mask, step,temperature=temperature,do_sample=do_sample)
        ...
        with torch.no_grad():
            ...
            seq_sentences = self.tokenizer.batch_decode(seq, skip_special_tokens=True)
            ...
            for i in range(len(prompts)):
                ...
                model_answer_reward_scores.append(
                    test_answer_for_reward(model_answer_sentence,answer[i],self.args.test_data_type)
                    )

            reward_score = torch.tensor(model_answer_reward_scores,dtype=type_dict[self.args.dtype])
            reward_score = reward_score.to(get_accelerator().current_device_name())
            # print(reward_score)
